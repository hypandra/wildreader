...
import { generateWordMatchQuestion } from "@/lib/games"
...
type WordMatchQuestion = ReturnType<typeof generateWordMatchQuestion>
...
export function WordMatchGame({
  ...
  useEffect(() => {
    if (question && audioEnabled && !rewardDialogOpen) {
      speakGameInstruction("word-match", question.target.word)
    }
  ...
  const generateQuestion = useCallback(() => {
    ...
    try {
      setQuestion(generateWordMatchQuestion())
    } catch (error) {
      console.error("Error generating word match question:", error)
  ...
  if (!question) {
    return (
      <div className="min-h-[320px] flex items-center justify-center">
        <p className="text-xl font-display text-bark">Loading word match...</p>
      </div>
    )
  }
    ...
    <div>
      <div className="text-center mb-4 animate-pop-in">
        <div className="inline-flex flex-col items-center gap-1">
          ...
          <AudioButton
            text={getGameInstruction("word-match", question.target.word)}
            size="sm"
            variant="ghost"
