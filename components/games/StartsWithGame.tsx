...
import { generateStartsWithQuestion } from "@/lib/games"
...
type StartsWithQuestion = ReturnType<typeof generateStartsWithQuestion>

export function StartsWithGame({
  vocabulary,
  onCorrect,
  onWrong,
  onComplete,
  ...
  const [question, setQuestion] = useState<StartsWithQuestion | null>(null)
  ...
  useEffect(() => {
    if (question && audioEnabled && !rewardDialogOpen) {
      speakGameInstruction("starts-with", question.target.word)
    }
  ...
  const generateQuestion = useCallback(() => {
    ...
    try {
      setQuestion((prevQuestion) => {
        ...
        const mastery = target?.mastery["starts-with"]
        const stage = mastery && getCorrectRate(mastery) > 0.8 && mastery.attempts >= 5 ? 2 : 1
        return generateStartsWithQuestion(stage as 1 | 2)
      })
    } catch (error) {
      console.error("Error generating starts with question:", error)
  ...
  if (!question) {
    return (
      <div className="min-h-[320px] flex items-center justify-center">
        <p className="text-xl font-display text-bark">Loading starts with...</p>
      </div>
    )
  }

  const showPicture = getCorrectRate(question.target.mastery["starts-with"]) <= 0.8 || question.target.mastery["starts-with"].attempts < 5
    ...
    <div>
      <div className="text-center mb-4 animate-pop-in">
        <div className="inline-flex flex-col items-center gap-1">
          ...
          <AudioButton
            text={getGameInstruction("starts-with", question.target.word)}
            size="sm"
            variant="ghost"
